...
#include "llama/llama_bios_settings.h" // llama_bios_settings
#include "llama/llama_boot.h" // llama_boot
#include "llama/llama_bios_menu.h" // llama_bios_menu
#include "llama/llama_boot_disk_strings.h"

// Helper function
int string_contains(const char *string, const char *substring) {
...
static llama_bios_settings lbs;
...
loadBootOrder(void)
{
    ...
    size_t f_size =
        sizeof(llama_boot_disk_string_sd) +
        sizeof(llama_boot_disk_string_usb) +
        sizeof(llama_boot_disk_string_sata) +
        sizeof(llama_boot_disk_string_vfloppy);

    char *f = malloc_tmphigh(f_size);

    switch (lbs.boot_order_index.value) {
      case 0: // sata-sd-usb-vfloppy
        snprintf(f, f_size, "%s%s%s%s",
            llama_boot_disk_string_sata,
            llama_boot_disk_string_sd,
            llama_boot_disk_string_usb,
            llama_boot_disk_string_vfloppy
        );
        break;
      case 1: // sata-usb-sd-vfloppy
        snprintf(f, f_size, "%s%s%s%s",
            llama_boot_disk_string_sata,
            llama_boot_disk_string_usb,
            llama_boot_disk_string_sd,
...
boot_init(void)
{
    ...
    lbs = llama_bios_settings_default;
    llama_load_bios_settings(&lbs);
...
{
    if (lbs.usb_as_fixed_disks.value == 0) {
        if (string_contains(desc, "USB MSC")) return;
    }
...
interactive_bootmenu(void)
{
    ...
    int boot_menu_key = 0x01;  // ESC
    printf("\nPress F1 for Setup, ESC for Boot Menu\n");
